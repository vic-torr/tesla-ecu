...
uint16_t slip[2], slip_ratio[2],
					roda_interna;	// wheel slip de cada roda traseira e a razao entre o obtido e o desejado
									// variavel de controle da vetorizacao
...
void testa_comunic_inv(uint16_t com_inv_periodo_ms){
	...
	if (com_inv_milis - com_inv_milis_ant >= com_inv_periodo_ms) {
		...
		if(flag_msg_ant_inversor[MOTOR_ESQ] == false
				|| flag_msg_ant_inversor[MOTOR_ESQ] == false){
...
uint16_t le_acelerador(uint8_t *flag_error) {
	...
	if (((acelerador > 300 && BSE > 2200) || (BSE < 600))) {
		*flag_error = 1;
		acelerador = 0;
	}
	if (*flag_error == 1) {
...
void controle() {
	...
	else if ((acelerador < 5) && (frenagem_regenerativa == true)
		 &&	(vel_motor[MOTOR_DIR] > _5_kmph_rpm || vel_motor[MOTOR_ESQ] > _5_kmph_rpm))
	{
		estado = FREIA;
	}
	else if(acelerador>300)
	{
		 estado = ACELERA;
	}
	else if (botao_habilita_acionado() && ADC_DMA[5] > SELECAO_5)
	{
		estado = REVERSE;
	}
	else
...
void actual_datalogger() {
	...
	if (flag_dtl == 2) //variï¿½vel "flag_dtl" assume o valor "2" no final da main
	{
		vetTx[0] = vel_roda[RODA_DIANT_DIR];
		vetTx[1] = vel_roda[RODA_DIANT_DIR] >> 8;
		vetTx[2] = vel_roda[RODA_DIANT_ESQ];
		vetTx[3] = vel_roda[RODA_DIANT_ESQ] >> 8;
...
void transmite_ecu_datalogger() {
	if (modo_selecionado.mode == ACELERACAO)
		acel_datalogger();
	else if (modo_selecionado.mode == SKIDPAD)
		skid_datalogger();
	else if (modo_selecionado.mode == AUTOX)
		autox_datalogger();
	else if (modo_selecionado.mode == ENDURO)
		enduro_datalogger();
}


// cada datalogger manda dados de acordo com a tabela dos IDs que tem no drive
...
void enduro_datalogger() {
	if (flag_dtl == 2) {
		...
		vetTx[2] = torque[MOTOR_ESQ];
		vetTx[3] = torque[MOTOR_ESQ] >> 8;
		vetTx[4] = vel_motor[MOTOR_DIR];
		vetTx[5] = vel_motor[MOTOR_DIR] >> 8;
		vetTx[6] = vel_motor[MOTOR_ESQ];
		vetTx[7] = vel_motor[MOTOR_ESQ] >> 8;
		...
		vetTx[0] = temp1_mosf[MOTOR_DIR];
		vetTx[1] = temp1_mosf[MOTOR_DIR] >> 8;
		vetTx[2] = temp1_mosf[MOTOR_ESQ];
		vetTx[3] = temp1_mosf[MOTOR_ESQ] >> 8;
		vetTx[4] = tempo_teste;
		vetTx[5] = tempo_teste >> 8;
		vetTx[6] = dist_calc;
		vetTx[7] = dist_calc >> 8;
		CANSPI_Transmit(0x306, 8, vetTx);
		flag_dtl = 2;
...
void init_datalogger() {
	...
	vetTx[6] = 0;
	vetTx[7] = 0;
	check_error = CANSPI_Transmit(0x310, 8, vetTx);
...
void comando_inversor() {
	paramControl = (habilita) + (runstop << 1) + (freiar << 2)
					+ (acelerar << 3) + (rev << 4);
	...
	// tirar duas posiï¿½oes do error_log
	delay_ms_ecu(15);
...
uint16_t wheel_slip(int selector) {
	...
	uint16_t s = (uint8_t) (100*( vel_roda[selector] - media_diant)/ media_diant);
	return s;
}
