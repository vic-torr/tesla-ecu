...
uint16_t le_volante() {
	...
	if (acelerador < 300) acelerador = 0;
...
void controle() {
	...
	//else
	//		if ((acelerador < 80) && (frenagem_regenerativa == true) && (regen_bk_selection == true)
	 //&&	 vel_motor[MOTOR_ESQ_DIR] > _5_kmph_rpm)
...
void actual_datalogger() {
	if (flag_can == 1) //variï¿½vel "flag_dtl" assume o valor "2" no final da main
	{
		...
		vetTx[0] = (uint16_t) time_actual;	//Converte para um int 16
		vetTx[1] = (uint16_t) time_actual >> 8;
		vetTx[2] = volante_cru;
		vetTx[3] = volante_cru >> 8;
		vetTx[4] = acelerador;
	...
	// IDs para debug usando painel
		...
		vetTx[6] = 0;
		vetTx[7] = 0;
		check_error = CANSPI_Transmit(350, 8, vetTx);
		flag_can = 1;
	}
}
...
{
	if (dist_pr >= 10) { //conta ate 1s
		dist_pr = 0;		//reinicia contagem de tempo
